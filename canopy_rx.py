...
# Safe import for browser-based GPS coordinate values
try:
...
def fetch_environmental_data(latitude, longitude):
    ...
    # Default fallback data structure in case endpoints are timed out
    data = {
        "temp": 27.0, "humidity": 70.0, "uv": 5.0,
        "pm25": 35.0, "pm10": 50.0, "no2": 20.0, "co": 380.0, "o3": 40.0
    }
    try:
        # Fetch standard weather metrics
        ...
        # Fetch active air quality indices
        ...
        pass  # Gracefully fall back to defaults if server network dips
    return data
...
# --- 5. SIDEBAR GEOGRAPHIC & ELIGIBILITY CONTROLS ---
...
# --- LIVE BROWSER GEOLOCATION LINK ---
if st.sidebar.button("📌 Detect My Live Location", use_container_width=True):
    try:
        loc_str = streamlit_js_eval(
            data_of='navigator.geolocation.getCurrentPosition((pos) => {return pos.coords.latitude + "," + pos.coords.longitude})', 
            target_id='gps_fetch'
        )
        if loc_str:
            ...
            st.session_state.engine_active = True
            st.toast("🎯 Real-time coordinates retrieved successfully!", icon="📍")
    except Exception:
        st.sidebar.error("Please verify browser location settings are set to 'Allow'.")
...
# Automatically reverse-resolve nice looking text addresses when coordinate values change
if resolved_by_coords:
    try:
        geolocator = Nominatim(user_agent="canopy_rx_platform_engine")
...
if st.session_state.engine_active:
    # Trigger active API data extraction
    ...
    # =========================================================================
    # 🌍 SEED 1: FREE PUBLIC TIER (TRAVEL CONDITIONS & BIO-CARE ADVICE)
    # =========================================================================
    st.markdown("### <span class='free-badge'>FREE PUBLIC ACCESS</span> ✈️ Eco-Travel & Bio-Care Advisory", unsafe_allow_html=True)
    st.write("Dynamic packing recommendations and protective shielding tips customized to local climate attributes:")
    ...
    # =========================================================================
    # 💎 SEED 2: PREMIUM SUBSCRIPTION PAYWALL GATING ENGINE
    # =========================================================================
    st.write("---")
    ...
    if st.session_state.user_tier == "Premium":
        ...
        # Show marketing lock-out banner if account tier eligibility falls to Free
        st.markdown("""
    ...
    # =========================================================================
    # 📊 REFERENCE EXPLANATIONS AND DECODED VALUE BASES
    # =========================================================================
    st.write("---")
    st.markdown("### 🔬 Scientific Environmental Metrics & Decoded Reference Baselines")
    st.caption("Expand any parameter category box below to check standard healthy baselines and explanations of what the parameters mean.")
    ...
    with tab_particulates:
        ...
        with col_p1:
            ...
            with st.expander("📝 Deconstruct PM2.5 & Normal Values"):
                st.markdown("""
                * **What it means:** Microscopic airborne dust particles smaller than 2.5 micrometers suspended in the breathing column.
                * **Why it matters:** They effortlessly sneak past nasal hairs, settling directly inside deep lung cells or entering blood vascular tracks.
        ...
        with col_p2:
            ...
            with st.expander("📝 Deconstruct PM10 & Normal Values"):
                st.markdown("""
                * **What it means:** Coarser ash, soil dust, dust mite fragments, and pollen tracking between 2.5 and 10 micrometers.
                * **Why it matters:** Irritates airway walls, driving cough reflexes, itchy throats, and acute sinus inflammation.
    ...
    with tab_gases:
        ...
        with col_g1:
            ...
            with st.expander("📝 Deconstruct NO₂"):
                st.markdown("""
                * **What it means:** A pungent gas resulting from heavy vehicle exhaust and urban fossil fuel burning.
                * **Why it matters:** Severely compromises lung defense cells, increasing your vulnerability to airborne allergies.
        ...
        with col_g2:
            ...
            with st.expander("📝 Deconstruct CO"):
                st.markdown("""
                * **What it means:** An odorless gas from engine combustion.
                * **Why it matters:** Competes with oxygen bindings inside blood tracking, causing fatigue or head tension.
        ...
        with col_g3:
            ...
            with st.expander("📝 Deconstruct Ozone"):
                st.markdown("""
                * **What it means:** Surface-level smog cooked by direct solar light interacting with exhaust emissions.
                * **Why it matters:** Acts like an oxidative stress factor on raw throat tissue linings.
                * **Normal/Healthy Values:** <span class="normal-value-badge">Safe Threshold: Below 100 µg/m³ over 8 hours</span>
                """, unsafe_allow_html=True)

    # --- 7. INTERACTIVE LOCATION MAP RENDER ---
    st.write("---")

...
# ==========================================
def generate_gemini_clinical_insight(prompt_context, gemini_api_key=None):
    try:
        if gemini_api_key:
            genai.configure(api_key=gemini_api_key)
        else:
            # Fallback or stream state if stored
            if "gemini_key" in st.session_state and st.session_state.gemini_key:
                genai.configure(api_key=st.session_state.gemini_key)
            else:
                return "⚠️ Please configure your Gemini API Key in the sidebar to enable live AI clinical synthesis."
...
# ==========================================
# 🗺️ SIDEBAR NAVIGATION & GEMINI SETTINGS
# ==========================================
...
st.sidebar.write("---")
st.sidebar.markdown("### 🤖 Gemini AI Configuration")
gemini_input_key = st.sidebar.text_input("Gemini API Key:", type="password", value=st.session_state.get("gemini_key", ""))
if gemini_input_key:
    st.session_state.gemini_key = gemini_input_key
st.sidebar.info("Provide your Gemini API key to enable live AI-powered clinical synthesis across all portal modules.")
...
# ==========================================
    ...
    with col_map:
        ...
        st.info("💡 **Note on Map Labeling ('Main'):** The label appearing on the map is generated by OpenStreetMap tiles representing a local neighborhood, road, or landmark designation.")
